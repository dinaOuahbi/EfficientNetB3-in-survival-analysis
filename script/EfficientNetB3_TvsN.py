...
# des tuiles Tumorales vs Normales sur la DB5
# -------------------------------------------------------------
"""
...
print("\n\n+++++ Running CNN EfficientNetB3_CNN2+++++\n\n")
...
train_datagen = ImageDataGenerator(
        ...
        fill_mode="nearest")
train_dir = "../TCGA_annot_DB11/Train"
val_dir = "../TCGA_annot_DB11/Val"
test_dir = "../TCGA_annot_DB11/Test"
...
base_model.trainable = True  # On veut entrainer tout les poids
...
# On calcule le poids de chacune des deux classes
...
model.save('../model/NvsT_EfficientNetB3')
model.save('../model/NvsT_EfficientNetB3.h5')

hist_df = pd.DataFrame(history.history)
hist_csv_file = '../model/NvsT_EfficientNetB3_history.csv'
with open(hist_csv_file, mode='w') as f:
    hist_df.to_csv(f)

output = '../Resultats/NvsT_DB11/'

# Predictions
# ATTENTION : shuffle doit etre a faux pour que les filenames correspondent bien aux images vues par le generateur
...
myPrediction.to_csv(output+"myPrediction_train_NvsT.csv", sep=',', encoding='utf-8',
...
myPrediction = test_predictions_baseline
myPrediction = pd.DataFrame(myPrediction, index = myName)
myPrediction.to_csv(output+"myPrediction_val_NvsT.csv", sep=',', encoding='utf-8',
               index=True, header = None)
...
myPrediction = test_predictions_baseline
myPrediction = pd.DataFrame(myPrediction, index = myName)
myPrediction.to_csv(output+"myPrediction_test_NvsT.csv", sep=',', encoding='utf-8',
               index=True, header = None)
...
myPrediction = test_predictions_baseline
myPrediction = pd.DataFrame(myPrediction, index = myName)
myPrediction.to_csv(output+"myPrediction_Besancon_NvsT.csv", sep=',', encoding='utf-8',
               index=True, header = None)
